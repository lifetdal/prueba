...
def delete_app():
    ...
    log_message(f"[auto-stop] - Deleting app {app_type}-{app_name}. Domain ID: {domain_id}. Space name: {space_name}. Resource ARN: {resource_arn}.")
...
if __name__ == '__main__':
    ...
    state_file_path = "/var/tmp/auto-stop/autostop.st"

    # Read in command-line parameters
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ht:p:k:u:c:s:a:", ["help","idle-time=", "datetime=", "port=","hostname=","base-url=","ignore-connections=", "skip-terminals=", "state-file-path="])
        if len(opts) == 0:
    ...
    if idle_time:
        ...
        if now.time() >= any_day_at_time:
            ...
            log_message("[auto-stop] - JupyterLab has not reached the scheduled time. Passing check.")
            exit(0)
    else:
        log_message("[auto-stop] - Missing '-t', '--idle_time' , '-d' or '--datetime'")
        exit(2)
